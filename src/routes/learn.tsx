...
function LearnPage() {
  ...
  return <div className="space-y-6">
    <PageHeader eyebrow="Concept studio" title="Learn by doing" description="Short, structured lessons that move from concept to example to an executable experiment." actions={<div className="relative w-full sm:w-72"><Search className="absolute left-3 top-2.5 size-4 text-muted-foreground" /><Input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter this unit…" className="pl-9" /></div>} />
    ...
    <div className="grid gap-6 lg:grid-cols-[250px_1fr]">
      <aside className="panel h-fit p-3"><p className="px-2 pb-2 text-xs font-semibold uppercase tracking-wider text-muted-foreground">{visible.length} lessons</p>{visible.map((l) => <Link key={l.id} to={`/learn/${l.id}` as never} className="flex items-center gap-2 rounded-lg px-2 py-2 text-sm hover:bg-muted"><span className="shrink-0">{progress.completedLessons.includes(l.id) ? <CheckCircle2 className="size-4 text-success" /> : <Circle className="size-4 text-muted-foreground/60" />}</span><span className="truncate">{l.title}</span></Link>)}</aside>
